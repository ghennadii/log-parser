...
int main (int argc, char* argv[])
{
   ...
   if (argc != 2) {
      cerr << "Usage: aggregate <filename>" << endl;
      return -1;
   }
   ...
   int lineNumber = 0, parseErrorCount;
   double result;
   ...
   while (getline(fin, line)) {
      lineNumber++;
      if(!parseLine(line, function, args, supportedFunctions, error)) {
         parseErrorCount++;
         printError(lineNumber, error);

         //Exit the program if consecutives parse error count reach 100
         if (parseErrorCount == 100) {
            cout << "100 lines have consecutive error, please provide valid text file." << endl;
            break;
         }
      ...
      //Reset errorCount, because it calculates only consecutives
      parseErrorCount = 0;

      result = apply(supportedFunctions, function, args, error);

      if (error != NO_ERROR) {
         printError(lineNumber, error);
      }

      cout << "Line " << lineNumber << ": " << result << endl;
   }
